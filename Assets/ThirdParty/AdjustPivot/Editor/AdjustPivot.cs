...
public class AdjustPivot : EditorWindow
{
        ...
        if (!IsNull(selection))
        {
            if (!IsNull(selection.parent))
            {
                if (selection.localPosition != Vector3.zero || selection.localEulerAngles != Vector3.zero)
                {
                    ...
                    if (GUILayout.Button("Check <b>" + selection.parent.name + "</b>'s center", buttonStyle, buttonHeight))
                    {
                        var renderers = selection.parent.GetComponentsInChildren<Renderer>();
                        ...
                        center /= count;
                        selection.localPosition = center;
                    }
                }
                else
                {
    ...
    {
        var pType = PrefabUtility.GetPrefabAssetType(obj);
        return pType == PrefabAssetType.Regular || pType == PrefabAssetType.Variant || pType == PrefabAssetType.Model;
    }
